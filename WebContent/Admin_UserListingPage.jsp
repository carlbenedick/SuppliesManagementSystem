...
<body>
	<div id="userTable">
		<table id="addData">
			...
			<tr>
				<th>Active Tag</th>
				<th><input type="radio" id="activeTags" checked="checked"
					value="Y">Yes <input type="radio" id="activeTag"
					checked="checked" value="N">No</th>
			</tr>
			<tr>
				<th>Access Level</th>
		...
		<div>
			<!-- 			<fieldset id="dataTable"
				style="width: 1100px; border: 1px solid #ffe6e6; padding: 10px; align: center;"> -->
...
<script>
		$("activeTags").checked = false;
		$("activeTag").checked = false;
		var inserts = $("tableRow");
		for (var i = 0; i < inserts.rows.length; i++) {
			inserts.rows[i].onclick = function() {

				$("user_id").value = this.cells[0].innerHTML;
				$("firstName").value = this.cells[2].innerHTML;
				$("lastName").value = this.cells[3].innerHTML;
				$("middleInitial").value = this.cells[4].innerHTML;
				$("emailAdd").value = this.cells[5].innerHTML;
				if (this.cells[6].innerHTML == "Y") {
					$("activeTags").checked = true;
					$("activeTag").checked = false;
				} else if (this.cells[6].innerHTML == "N") {
					$("activeTag").checked = true;
					$("activeTags").checked = false;
				}
				$("accessLevel").value = this.cells[7].innerHTML;
			}
	}
	...
	function updateProfiles() {
		new Ajax.Request(contextPath + "/updateProfile", {
			...
			parameters : {
				...
				email : $F("emailAdd"),
				activeTag : $F("activeTag"),
				activeTag : $F("activeTags"),
				accessLevel : $F("accessLevel")
			},
			onComplete : function(response) {
				 $("userTable").update(response.responseText);
			/* 	 window.location.reload("/maintenance?action=maintain");  */
			}
		});
	} 
	/* $("btnChangePassword").observe("click", function() {
		new Ajax.Request(contextPath + "/changepassword", {
			method : "GET",
			parameters : {
